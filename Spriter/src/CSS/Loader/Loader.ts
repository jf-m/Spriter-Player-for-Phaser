module Spriter {
    ...
    export class Loader {
        ...
        private _loaderPromise: Promise<Object>;
        ...
        // -------------------------------------------------------------------------
        public load(file: SpriterFile): Promise<Object> {
            ...
            return new Promise((resolve): void => {
                ...
                this.loadFolders(this._spriter, folders, resolve);
                folders.processed();
        ...
        // -------------------------------------------------------------------------
        private loadFolders(spriter: Spriter, folders: ISpriterNodeList, resolve): void {
            let allFoldersProcessed = [];
            // through all folders
            for (var i = 0; i < folders.length(); i++) {
                ...
                allFoldersProcessed.push(this.loadFiles(folder, files));
                files.processed();

                // add folder to spriter object
                spriter.addFolder(folder);
            }
            Promise.all(allFoldersProcessed).then( () => {
                resolve(this._spriter);
            } );
        }

        // -------------------------------------------------------------------------
        private loadFiles(folder: Folder, files: ISpriterNodeList): Promise<Object> {
                ...
                for (var f = 0; f < files.length(); f++) {
                    ...
                    if (file !== null) {
                        if (file.isEmpty !== true) {
                            ...
                            img.src = this._assetsPath + file.name;
                            */
                            resolve();
                        }
                        folder.addFile(file);
