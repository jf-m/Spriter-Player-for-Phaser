...
(function (Spriter) {
    class Loader {
        ...
        // -------------------------------------------------------------------------
        loadFolders(spriter, folders, resolve) {
            ...
            Promise.all(allFoldersProcessed).then(resolve);
        }
        // -------------------------------------------------------------------------
                ...
                for (var f = 0; f < files.length(); f++) {
                    ...
                    if (file !== null) {
                        if (file.isEmpty !== true) {
                            ...
                            img.src = this._assetsPath + file.name + ".svg";
                        }
                        folder.addFile(file);
                    }
                }
...
(function (Spriter) {
    class SpriterGroup {
        // -------------------------------------------------------------------------
        constructor(container, spriter, texutreKey, entityName, animation, animationSpeedPercent, isUsingAtlas) {
            // onLoop(SpriterGroup);
        ...
        setObjects(objects, force = false) {
            ...
            for (var i = 0; i < objects.length; i++) {
                ...
                if (this._objects[ref.id] === undefined) {
                    ...
                    sprite.className = 'body-part-object ' + 'object-' + object.timeline;
                ...
                if (object.type === 0 /* SPRITE */) {
                    object.setOn(true);
                    if (parseInt(object.sprite.style.zIndex) !== ref.z) {
                        object.sprite.style.zIndex = "" + ref.z;
                        zChange = true;
                    }
                }
                else {
...
(function (Spriter) {
    class SpriterObject extends Spriter.SpriterBone {
        ...
        setOn(on, hideSprite = false) {
            ...
            this._sprite.style.display = (on && !this._hide && !hideSprite) ? 'block' : 'none';
        }
        // -------------------------------------------------------------------------
        setKey(entity, animation, timelineId, keyId) {
        ...
        setFile(file) {
            ...
            if (file !== null && file.isEmpty !== true) {
                this._hide = false;
                this._sprite.style.backgroundImage = 'url(' + file.name + ')';
            }
            else {
                this._hide = true;
